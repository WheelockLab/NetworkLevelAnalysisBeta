function pairsOut=View_Clear_Brain_ROI_Corr2a_simple(Anat,IM,M,dataOut,fc,Pmax,params)
%
...
roi.radius=repmat(5,Nrois,1); %%## Set radius (volume?) to proportional
...
for k=1:Nrois
    ...
    roi.radius(k,1)=4+sum(pairsOut{j,ts}(:)==Rs(k)); % Scale Rad, Area, Vol?
    end
end
...
for k=1:Npairs
    ...
    pairsOut{j,ts}(k,5)=min(fc);
    pairsOut{j,ts}(k,6)=max(fc);
    pairsOut{j,ts}(k,7)=(mean(fc)/std(fc))*sqrt(length(fc)-1);
    pairsOut{j,ts}(k,8)=mean(fc);
    pairsOut{j,ts}(k,9)=std(fc);
    pairsOut{j,ts}(k,10)=median(fc);
    Ns=length(fc);
    sfc=sort(fc);
    pairsOut{j,ts}(k,11)=sfc(round(Ns/4));
    pairsOut{j,ts}(k,12)=sfc(round(3*(Ns/4)));
...
% figure('Color','w');fcBx_PercFC_Scat4(pairsOut{ts},IM); % mean(fc)
% figure('Color','w');fcBx_PercFC_Scat5(pairsOut{ts},IM); % median b&w
end
