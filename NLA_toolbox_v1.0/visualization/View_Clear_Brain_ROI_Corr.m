function pairsOut=View_Clear_Brain_ROI_Corr(Anat,IM,M,Pnn,Proi,C,Pmax,params,g)
%
...
for j=1:NnnSig
    ...
    if params.toDraw
disp(['Drawing ROIs for ',IM.Nets{NNsig(j,1)},' and ',IM.Nets{NNsig(j,2)}])
    if ~isempty(pairs)
...
% ## to number of connections to roi in a figure...
...
for k=1:Nrois
    ...
    temp = radius+sum(pairs(:)==Rs(k));
        if temp<=10;
    roi.radius(k,1)=radius+sum(pairs(:)==Rs(k)); % Scale Rad, Area, Vol?
        else
            roi.radius(k,1)=10;
        end
    end
