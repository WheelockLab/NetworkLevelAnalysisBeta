function dataOut=fcBx_Chi_McNemar_TwoGroups(fc,Bx,IM,params)
...
%
% This function runs the permutation analysis for multiple fcBx enrichment
% analyses in parallel so that they maintain their relevant correlation 
% structure. Each set of initial data is contained in a cell of fc and Bx.
%
% _mixedLCS stands for mixed longitudinal cross-sectional. In other words,
% the data is assumed to be composed of (e.g.) data for 2 groups (12mo and
% 24mo). This version of the code does not have functionality for mixed
% longitudinal data
%
% fc{1} and Bx{1} cross-sectional data for group 1
% fc{2} and Bx{2} cross-sectional data for group 2
%
% The primary analysis focusses on Enrichment Analysis for the full set of
% subjects group 1 and (separately) group 2, as well as the
% McNemar statistic between those sets of data. Permutation-based
% significance is correlated separately for those cases.
%
% While output data is structured into 2 levels (Enrichment for group
% 1 and 2 and then a separate set of arrays for the McNemar stat between 
% them), the 2 data sets are maintained for randomization.
% 
...
dataOut.thresholdedROIpairs=zeros(Nroi*Nroi,Nsets,'single');
...
dataOut.McNemar_stats=ones(Nnets*Nnets,1,'single');
dataOut.TNidx=TNidx;
...
switch type
  ...
  % - randomize within groups to maintain correlation structure....
    case 'Spearman'
        [rho,pval0i]=MySpearman(Bx{j}, fc{j}');
    case 'Pearson'
        [rho,pval0i]=corr(fc{j}',Bx{j},'type','Pearson');
end

dataOut.rho(Tidx,j)=rho;
...
[dataOut.McNemar_P(TNidx),dataOut.McNemar_b(TNidx),...
    dataOut.McNemar_c(TNidx),dataOut.McNemar_stats(TNidx)]=McNemar_fast(McNs,NNidx);
...
for j=1:Nsets
    ...
    parfor j=1:Ntests
        ...
        %randL=randperm(Nbx(1)); % Longitudinal randomization
        McNs=zeros(Nroi_pairs,2);
        for G=1:Nsets
             randL=randperm(Nbx(G)); % Longitudinal randomization
            switch type
                case 'Spearman'
                    [~,pval0p]=MySpearman(Bx{G}(randL),fc{G}');
                case 'Pearson'
                    [~,pval0p]=corr(fc{G}',Bx{G}(randL));%
            end
...
%             Es=round(N_exp_sig_ratio_tot*Np);
    ...
    for j=1:Npairs
        ...
        if k==1, title(params.group{1})
        elseif k==2, title(params.group{2})
        end
        subplot(2,3,3+k)
...
dataOut.McNemar_stats=reshape(dataOut.McNemar_stats,Nnets,Nnets);
